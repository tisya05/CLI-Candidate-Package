...
from interview_client.shared_utils.client_config import SERVER_URL, LOCAL_FOLDER, IGNORED_FILES, IGNORED_EXTENSIONS
...
def run(args):
    ...
    print("[INFO] Collecting files for test...")
    ...
    print("[INFO] Sending code to server for local test run...")

    files = {f: open(os.path.join(local_folder, f), 'rb') for f in filtered_files}
    files["session.json"] = open(session_path, 'rb')

    try:
        response = requests.post(f"{SERVER_URL}/test_local", files=files)
    finally:
    ...
    for r in results:
        ...
        print("==> Result:", "PASSED" if r["passed"] else "FAILED")
        print()
    ...
    report_path = get_next_report_path(reports_dir, "local")

    with open(report_path, "w") as f:
        json.dump(report_data, f, indent=2)

    print(f"\n[INFO] Local test report saved at:\n>> {report_path}")
    ...
    print("[SUCCESS] Local test run complete.\n")
